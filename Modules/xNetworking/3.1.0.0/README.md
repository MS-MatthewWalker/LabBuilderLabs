...
- **`[String]` IPAddress** (_Key_): The desired IP address.
- **`[String]` InterfaceAlias** (_Key_): Alias of the network interface for which the IP address should be set.
- **`[Uint32]` PrefixLength** (_Write_): The prefix length of the IP Address.
- **`[String]` AddressFamily** (_Write_): IP address family. { *IPv4* | IPv6 }. Defaults to IPv4.

### xDnsServerAddress

- **`[String]` InterfaceAlias** (_Key_): Alias of the network interface for which the DNS server address is set.
- **`[String]` AddressFamily** (_Key_): IP address family. { *IPv4* | IPv6 }. Defaults to IPv4.
- **`[String[]]` Address** (_Required_): The desired DNS Server address(es).
- **`[Boolean]` Validate** (_Write_): Requires that the DNS Server addresses be validated if they are updated. It will cause the resouce to throw a 'A general error occurred that is not covered by a more specific error code.' error if set to True and specified DNS Servers are not accessible. Defaults to False.

### xDnsConnectionSuffix

- **`[String]` InterfaceAlias** (_Key_): Alias of the network interface for which the DNS server address is set.
- **`[String]` ConnectionSpecificSuffix** (_Required_): DNS connection-specific suffix to assign to the network interface.
- **`[Boolean]` RegisterThisConnectionsAddress** (_Write_): Specifies that the IP address for this connection is to be registered. Defaults to True.
- **`[Boolean]` UseSuffixWhenRegistering** (_Write_): Specifies that this host name and the connection specific suffix for this connection are to be registered. Defaults to False.
- **`[String]` Ensure** (_Write_): Ensure that the network interface connection-specific suffix is present or not. { *Present* | Absent }. Defaults to Present.

### xDefaultGatewayAddress

- **`[String]` InterfaceAlias** (_Key_): Alias of the network interface for which the default gateway address is set.
- **`[String]` AddressFamily** (_Key_): IP address family. { *IPv4* | IPv6 }. Defaults to IPv4.
- **`[String]` Address** (_Write_): The desired default gateway address - if not provided default gateway will be removed.

### xFirewall

- **`[String]` Name** (_Key_): Name of the firewall rule.
- **`[String]` DisplayName** (_Write_): Localized, user-facing name of the firewall rule being created.
- **`[String]` Group** (_Write_): Name of the firewall group where we want to put the firewall rule.
- **`[String]` Ensure** (_Write_): Ensure that the firewall rule exists. { *Present* | Absent }. Defaults to Present.
- **`[String]` Enabled** (_Write_): Enable or Disable the supplied configuration. { True | False }
- **`[String]` Action** (_Write_): Allow or Block the supplied configuration. { NotConfigured | Allow | Block }
- **`[String[]]` Profile** (_Write_): Specifies one or more profiles to which the rule is assigned.
- **`[String]` Direction** (_Write_): Direction of the connection. { Inbound | Outbound }
- **`[String[]]` RemotePort** (_Write_): Specific port used for filter. Specified by port number, range, or keyword.
- **`[String[]]` LocalPort** (_Write_): Local port used for the filter.
- **`[String]` Protocol** (_Write_): Specific protocol for filter. Specified by name, number, or range.
- **`[String]` Description** (_Write_): Documentation for the rule.
- **`[String]` Program** (_Write_): Path and filename of the program for which the rule is applied.
- **`[String]` Service** (_Write_): Specifies the short name of a Windows service to which the firewall rule applies.
- **`[String]` Authentication** (_Write_): Specifies that authentication is required on firewall rules. { NotRequired | Required | NoEncap }
- **`[String]` Encryption** (_Write_): Specifies that encryption in authentication is required on firewall rules. { NotRequired | Required | Dynamic }
- **`[String[]]` InterfaceAlias** (_Write_): Specifies the alias of the interface that applies to the traffic.
- **`[String]` InterfaceType** (_Write_): Specifies that only network connections made through the indicated interface types are subject to the requirements of this rule. { Any | Wired | Wireless | RemoteAccess }
- **`[String[]]` LocalAddress** (_Write_): Specifies that network packets with matching IP addresses match this rule. This parameter value is the first end point of an IPsec rule and specifies the computers that are subject to the requirements of this rule. This parameter value is an IPv4 or IPv6 address, hostname, subnet, range, or the following keyword: Any.
- **`[String]` LocalUser** (_Write_): Specifies the principals to which network traffic this firewall rule applies. The principals, represented by security identifiers (SIDs) in the security descriptor definition language (SDDL) string, are services, users, application containers, or any SID to which network traffic is associated.
- **`[String]` Package** (_Write_): Specifies the Windows Store application to which the firewall rule applies. This parameter is specified as a security identifier (SID).
- **`[String[]]` Platform** (_Write_): Specifies which version of Windows the associated rule applies.
- **`[String[]]` RemoteAddress** (_Write_): Specifies that network packets with matching IP addresses match this rule. This parameter value is the second end point of an IPsec rule and specifies the computers that are subject to the requirements of this rule. This parameter value is an IPv4 or IPv6 address, hostname, subnet, range, or the following keyword: Any
- **`[String]` RemoteMachine** (_Write_): Specifies that matching IPsec rules of the indicated computer accounts are created. This parameter specifies that only network packets that are authenticated as incoming from or outgoing to a computer identified in the list of computer accounts (SID) match this rule. This parameter value is specified as an SDDL string.
- **`[String]` RemoteUser** (_Write_): Specifies that matching IPsec rules of the indicated user accounts are created. This parameter specifies that only network packets that are authenticated as incoming from or outgoing to a user identified in the list of user accounts match this rule. This parameter value is specified as an SDDL string.
- **`[String]` DynamicTransport** (_Write_): Specifies a dynamic transport. { Any | ProximityApps | ProximitySharing | WifiDirectPrinting | WifiDirectDisplay | WifiDirectDevices }
- **`[String]` EdgeTraversalPolicy** (_Write_): Specifies that matching firewall rules of the indicated edge traversal policy are created. { Block | Allow | DeferToUser | DeferToApp }
- **`[String[]]` IcmpType** (_Write_): Specifies the ICMP type codes.
- **`[Boolean]` LocalOnlyMapping** (_Write_): Indicates that matching firewall rules of the indicated value are created.
- **`[Boolean]` LooseSourceMapping** (_Write_): Indicates that matching firewall rules of the indicated value are created.
- **`[Boolean]` OverrideBlockRules** (_Write_): Indicates that matching network traffic that would otherwise be blocked are allowed.
- **`[String]` Owner** (_Write_): Specifies that matching firewall rules of the indicated owner are created.

### xNetConnectionProfile

- **`[String]` InterfaceAlias** (_Key_): Specifies the alias for the Interface that is being changed.
- **`[String]` NetworkCategory** (_Write_): Sets the NetworkCategory for the interface per [the documentation](https://technet.microsoft.com/en-us/%5Clibrary/jj899565(v=wps.630).aspx). { Public | Private }
- **`[String]` IPv4Connectivity** (_Write_): Specifies the IPv4 Connection Value. { Disconnected | NoTraffic | Subnet | LocalNetwork | Internet }
- **`[String]` IPv6Connectivity** (_Write_): Specifies the IPv6 Connection Value. { Disconnected | NoTraffic | Subnet | LocalNetwork | Internet }

### xDhcpClient

- **`[String]` State** (_Key_): The desired state of the DHCP Client. { Enabled | Disabled }
- **`[String]` InterfaceAlias** (_Key_): Alias of the network interface for which the DNS server address is set.
- **`[String]` AddressFamily** (_Required_): IP address family. { IPv4 | IPv6 }

### xRoute

- **`[String]` InterfaceAlias** (_Key_): Specifies the alias of a network interface.
- **`[String]` AddressFamily** (_Key_): Specifies the IP address family. { IPv4 | IPv6 }
- **`[String]` DestinationPrefix** (_Key_): Specifies a destination prefix of an IP route. A destination prefix consists of an IP address prefix and a prefix length, separated by a slash (/).
- **`[String]` NextHop** (_Key_): Specifies the next hop for the IP route.
- **`[String]` Ensure** (_Write_): Specifies whether the route should exist. { *Present* | Absent }. Defaults to Present.
- **`[Uint16]` RouteMetric** (_Write_): Specifies an integer route metric for an IP route. Defaults to 256.
- **`[String]` Publish** (_Write_): Specifies the publish setting of an IP route. { *No* | Yes | Age }. Defaults to No.
- **`[Real64]` PreferredLifetime** (_Write_): Specifies a preferred lifetime in seconds of an IP route.

### xNetBIOS

- **`[String]` InterfaceAlias** (_Key_): Specifies the alias of a network interface.
- **`[String]` Setting** (_Required_): Default - Use NetBios settings from the DHCP server. If static IP, Enable NetBIOS. { Default | Enable | Disable }

### xNetworkTeam

- **`[String]` Name** (_Key_): Specifies the name of the network team to create.
- **`[String[]]` TeamMembers** (_Required_): Specifies the network interfaces that should be a part of the network team. This is a comma-separated list.
- **`[String]` TeamingMode** (_Write_): Specifies the teaming mode configuration. { *SwitchIndependent* | LACP | Static}. Defaults to SwitchIndependent.
- **`[String]` LoadBalancingAlgorithm** (_Write_): Specifies the load balancing algorithm for the network team. { Dynamic | *HyperVPort* | IPAddresses | MacAddresses | TransportPorts }. Defaults to HyperVPort.
- **`[String]` Ensure** (_Write_): Specifies if the network team should be created or deleted. { *Present* | Absent }. Defaults to Present.

### xNetworkTeamInterface

- **`[String]` Name** (_Key_): Specifies the name of the network team interface to create.
- **`[String]` TeamName** (_Required_): Specifies the name of the network team on which this particular interface should exist.
- **`[Uint32]` VlanID** (_Write_): Specifies VlanID to be set on network team interface.
- **`[String]` Ensure** (_Write_): Specifies if the network team interface should be created or deleted. { *Present* | Absent }. Defaults to Present.

### xHostsFile

- **`[String]` HostName** (_Key_): Specifies the name of the computer that will be mapped to an IP address.
- **`[String]` IPAddress** (_Write_): Specifies the IP Address that should be mapped to the host name.
- **`[String]` Ensure** (_Write_): Specifies if the hosts file entry should be created or deleted. { *Present* | Absent }. Defaults to Present.

### xNetAdapterBinding

- **`[String]` InterfaceAlias** (_Key_): Specifies the alias of a network interface. Supports the use of '*'.
- **`[String]` ComponentId** (_Key_): Specifies the underlying name of the transport or filter in the following form - ms_xxxx, such as ms_tcpip.
- **`[String]` State** (_Write_): Specifies if the component ID for the Interface should be Enabled or Disabled. { *Enabled* | Disabled }. Defaults to Enabled.
